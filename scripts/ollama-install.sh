...
(function () {
  ...
  // Desenha o cartao de uma chamada de ferramenta e devolve um atualizador.
  // 'result' existe quando a conversa esta sendo reaberta do localStorage;
  // quando a chamada esta acontecendo agora, o cartao nasce "executando" e o
  // localTool() chama o atualizador ao terminar.
  function drawToolCall(body, fn, result) {
    var box = document.createElement('div');
    box.className = 'toolcall';
    var stClass = result ? 'ok' : '';
    var stText = result ? 'executada' : 'executando...';
    box.innerHTML =
      '<div class="tc-head"><span class="badge">TOOL</span><span>' + esc(fn.name || '?') + '</span>' +
      '<span class="st ' + stClass + '">' + stText + '</span></div>' +
    ...
    return function (out) {
      ...
      var bad = out && out.error;
      st.textContent = bad ? 'erro' : 'executada';
      st.className = 'st ' + (bad ? 'bad' : 'ok');
      var args = document.createElement('div');
      args.className = 'tc-args';
      args.innerHTML = '<pre><code>' + esc(JSON.stringify(out, null, 2).slice(0, 2000)) + '</code></pre>';
  ...
  function runTools(model, calls, answer, thinkBuf) {
    ...
    var c = ensureChat();
    // Troca a bolha vazia do assistente (que so pediu a ferramenta) por uma que
    // carrega os cartoes. Guardo a referencia ANTES de tirar do DOM: appendChild
    // muda o no de pai, e sem isso o cartao ia parar na bolha errada.
    var nodes = wrap.querySelectorAll('.msg');
    var bubble = nodes.length ? nodes[nodes.length - 1] : null;
    if (bubble) wrap.removeChild(bubble);
    c.messages.pop();
    c.messages.push({ role: 'assistant', content: answer || '', thinking: thinkBuf || undefined, tool_calls: calls, ts: Date.now(), model: model });

    var host = null;
    if (bubble) {
      host = bubble.querySelector('.body');
      var content = bubble.querySelector('.content');
      if (content) content.innerHTML = '';
      if (thinkBuf) {
        var th = bubble.querySelector('.think');
        if (!th) drawThinking(host, thinkBuf);
      }
    ...
    var updaters = [];
    if (host) {
      calls.forEach(function (tc) { updaters.push(drawToolCall(host, tc.function || {}, null)); });
    }
    var pending = calls.map(function (tc, i) {
      var fn = tc.function || {};
      return localTool(fn.name, fn.arguments || {}).then(function (out) {
        if (updaters[i]) updaters[i](out);
